                ...
                <div class="row">
                    <div class="col-md-3">
                        <input type="text" name="search" class="form-control" 
                               placeholder="Cari event atau wisata..." 
                    ...
                    <div class="col-md-2">
                        <select name="wisata" class="form-control">
                            ...
                            @foreach($wisataList as $wisata)
                                <option value="{{ $wisata->id }}" {{ request('wisata') == $wisata->id ? 'selected' : '' }}>
                                    {{ $wisata->nama }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <tr>
                            <td>
                                <div class="d-flex px-2 py-1">
                                    <div>
                                        @if($event->poster)
                                            <img src="{{ asset($event->poster) }}" class="avatar avatar-sm me-3" alt="{{ $event->nama }}">
                                        @else
                                    ...
                                    <div class="d-flex flex-column justify-content-center">
                                        ...
                                        <p class="text-xs text-secondary mb-0">{{ Str::limit($event->deskripsi, 50) }}</p>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <p class="text-xs font-weight-bold mb-0">{{ $event->wisata->nama }}</p>
                                <p class="text-xs text-secondary mb-0">{{ Str::limit($event->wisata->alamat, 30) }}</p>
                            </td>
                            ...
                            <td class="align-middle text-center text-sm">
                                <span class="badge badge-sm bg-gradient-{{ 
                                    $event->status == 'aktif' ? 'success' : 
                                    ($event->status == 'selesai' ? 'secondary' : 'warning') 
                                }}">
                            ...
                            <td class="align-middle text-center">
                                <div class="btn-group" role="group">
                                    ...
                                    @if($event->status != 'aktif')
                                        <form action="{{ route('admin.event.update-status', $event->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="status" value="aktif">
                                            <button type="submit" class="btn btn-success btn-sm" data-bs-toggle="tooltip" title="Aktifkan">
                                    ...
                                    @if($event->status != 'selesai')
                                        <form action="{{ route('admin.event.update-status', $event->id) }}" method="POST" class="d-inline">
                                            ...
                                            <button type="submit" class="btn btn-secondary btn-sm" data-bs-toggle="tooltip" title="Tandai Selesai">
                                    ...
                                    <form action="{{ route('admin.event.destroy', $event->id) }}" method="POST" 
                                          class="d-inline" onsubmit="return confirm('Yakin ingin menghapus event ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm" data-bs-toggle="tooltip" title="Hapus Event">
